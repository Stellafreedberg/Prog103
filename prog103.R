...
#This code finds the number of cold hours each day where the air temperature is
#below -4 during late winter in the location Nuka Pass
...
#call the function cold_hours_per_day
#2 parameters called site and season

# P3 Write a function to encapsulate the code chunk above. Check that it
# contains all five parts of a function.
cold_hours_per_day <- function(site,season) {
  n_cold <- sum(kefj_site ==site &
                  kefj_season == season &
                  kefj_temperature <= -4 &
                  kefj_exposure == "air")
...
cold_hours_per_day("Nuka_Pass","Late winter")
...
extreme_hours_per_day("Aialik","Late winter", "cold")
# Season to taste ---------------------------------------------------------

# P7 What seasons are in the kefj dataset? What function would you use to
# identify them?
#Late winter,spring, summer, fall, early winter. Use the season<- function to
#identify them
...
seasons <- c("Late winter", "Spring", "Fall", "Early winter")
  for (season in seasons) {
    heat_exposure <- extreme_hours_per_day( "Aialik", season, "hot")
    cold_exposure <- extreme_hours_per_day("Aialik",season, "cold")
